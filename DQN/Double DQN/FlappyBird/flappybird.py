...
class Args:
    ...
    buffer_size: int = 500000  # Reduced for CartPole
    gamma: float = 0.99  # Standard for shorter episodes
    tau: float = 1e-2  # Smooth target updates
    train_frequency: int = 2  # Train less often
    target_update_frequency: int = 10  # Update target less frequently
    learning_start: int = 20000  # Start training earlier
    ...
    log_interval: int = 5000  # Log more frequently
...
def make_env(env_id, seed, capture_video, run_name):
    ...
    if capture_video:
        env = gym.wrappers.RecordVideo(
            ...
            episode_trigger=lambda x: x % 5000 == 0,  # Save video for every episode
        )
