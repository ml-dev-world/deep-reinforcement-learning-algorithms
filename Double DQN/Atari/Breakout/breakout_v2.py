...
#     end_epsilon: float = (
#         0.1  # Higher end epsilon to retain more exploration in shorter runs
#     )
#     exploration_fraction: float = 0.2  # Faster decay over shorter timesteps
#     decay_rate: int = 20000  # Adjusted decay for 100k timesteps

#     # Training Settings
#     total_timesteps: int = 100000  # Updated to 100k
#     buffer_size: int = (
#         20000  # Reduced buffer size to better match shorter training period
#     )
#     gamma: float = 0.99
#     tau: float = 1.0
#     train_frequency: int = 4
#     target_update_frequency: int = (
#         500  # Update targets more frequently for faster learning
#     )
#     learning_start: int = 10000  # Reduced warm-up period for shorter training

#     # Model settings
#     batch_size: int = 32
#     learning_rate: float = 2e-4  # Slightly higher learning rate for faster updates
#     clip_grad_norm: int = 10
#     weight_decay: int = 0
#     gradient_steps: int = 1  # Reduced steps per training loop for efficiency
...
#     model_save_frequency: int = 20000  # Save models more frequently
#     log_interval: int = 5000  # Log more frequently for detailed monitoring
...
#     save_model: bool = True
#     model_dir: str = "saved_models"
#     model_save_frequency: int = 20000
...
def initialize_custom_replay_buffer(
    args, observation_space_shape, device: torch.device
):
    return ReplayBuffer(
...
def initialize_replay_buffer(
    buffer_size, observation_space, action_space, device, optimize_memory_usage
):
    return ReplayBuffer(
        buffer_size=buffer_size,
        observation_space=observation_space,
        action_space=action_space,  # Assuming discrete action space with 1 action dimension
        device=device,
        ...
        with tqdm(
            ...
            for global_step in pbar:
                ...
                if np.random.random() < epsilon:
                    ...
                    q_values = q_network(torch.from_numpy(state).to(device))
